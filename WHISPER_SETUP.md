...
1. Модель `small` автоматически скачается (~500MB)
2. Модель будет сохранена в кэш (обычно `~/.cache/whisper/`)
3. При последующих запусках модель загружается из кэша (быстро)

**Время первой загрузки:** зависит от скорости интернета (обычно 1-3 минуты)

**Место на диске:** ~500MB для модели `small`
...
```
INFO - Загружаем модель Whisper 'small' (первый запуск, это может занять время)...
INFO - Модель Whisper 'small' успешно загружена и кэширована
```
...
4. **Распознавание:**
   - Модель Whisper `small` распознаёт речь (модель загружается один раз и кэшируется)
...
- Это нормально для CPU (обычно 10-20 секунд для коротких сообщений)
...
- Текущая модель: `small` (хороший баланс скорости/качества для CPU)
